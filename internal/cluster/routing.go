...
func (p *routingPlan) shardCountForSlot(slot int) int {
	if p == nil {
		return 0
	}
	if p.ShardCount > 0 {
...
func (p *routingPlan) routeForKey(key []byte) (owner int, target int, migrating bool) {
	...
	shardCount := p.shardCountForSlot(slot)
	if shardCount <= 0 {
		shardCount = 1
	}
	...
	if assignment.Owner >= shardCount {
		assignment.Owner = assignment.Owner % shardCount
	}
	if assignment.Migrating {
		if assignment.Target < 0 {
			assignment.Target = assignment.Owner
		}
		if assignment.Target >= shardCount {
			assignment.Target = assignment.Target % shardCount
		}
...
func (p *routingPlan) normalize(shardCount int) {
	...
	for i := range p.Slots {
		...
		if p.Slots[i].Target < 0 || p.Slots[i].Target >= shardCount {
			p.Slots[i].Target = p.Slots[i].Owner
		}
	}
}
